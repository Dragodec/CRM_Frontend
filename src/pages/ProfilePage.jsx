...
export default function ProfilePage() {
  ...
  // Use useCallback to avoid React Hook missing dependency warning
  const loadProfile = useCallback(async () => {
    if (!userEmail) return;
    try {
      const data = await apiRequest(`users/profile/${userEmail}`, "GET", null, true);
  ...
  // PATCH request to toggle active status
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-start px-4 py-10 text-gray-200">
      <h1 className="text-4xl font-extrabold mb-8 text-purple-400">Profile</h1>
      ...
      >
        ...
        <div className="bg-gray-700 p-10 flex flex-col items-center justify-center space-y-6 border-r border-gray-600">
          ...
          <div className="flex items-center space-x-3 mt-6">
            {profile.active ? <FaCheckCircle className="text-green-500 text-2xl" /> : <FaTimesCircle className="text-red-500 text-2xl" />}
            <span className={`font-semibold text-lg ${profile.active ? "text-green-400" : "text-red-400"}`}>
...
function ErrorMessage({ message }) {
  return (
    <div className="p-6 bg-gray-900 text-gray-200 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-purple-400">Profile</h1>
